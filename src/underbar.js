...
(function() {
  ...
  _.each = function(collection, iterator) {
    if(Array.isArray(collection)) //Array iterator => (element, index, list)
    {
    ...
    else //OBJECT iterator => (value, key, list)
    {
  ...
  _.extend = function(obj) {
    ...
    {
      var objectOfKeys = arguments[i];

      for(var key in objectOfKeys)
      {
        obj[key] = objectOfKeys[key];
      }
    }
  ...
  _.memoize = function(func) {
    var alreadyCalledFunctions = new Array();

    return function()
    {
      var result;
      var alreadyCalled = false;
      for(var i=0; i<alreadyCalledFunctions.length; i++)
      {
        if(alreadyCalledFunctions[i][0] == arguments[0])
        {
          alreadyCalled = true;
          result = alreadyCalledFunctions[i][1];
          break;
        }
      }
      if(!alreadyCalled)
      {
        result = func.apply(this, arguments);
        alreadyCalledFunctions.push([arguments[0], result]);
      }
